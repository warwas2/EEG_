from src.data_preparation.data_preparation_overlapping90 import read_eeg_file
...
#    print(data.right_data.shape)# (165, 500, 3)
    ...
    for i in data.X:
        ...
        imgname="images_overlapping90/sub"+str(subject)+"_"+str(imgtype)+"_"+str(counts)+"_ch1.png"
...
#        plot.show()
        ...
        imgname="images_overlapping90/sub"+str(subject)+"_"+imgtype+"_"+str(counts)+"_ch2.png"
        plot.specgram(pd.DataFrame(i).iloc[:,1], NFFT=64, Fs=250, noverlap=32)
        plot.axis('off')
#        plot.show()
        plot.savefig(imgname, bbox_inches = "tight", pad_inches = 0, transparent = True)
        
        imgname="images_overlapping90/sub"+str(subject)+"_"+imgtype+"_"+str(counts)+"_ch3.png"
