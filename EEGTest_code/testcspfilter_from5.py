...
#    counts=0
    ...
    for i in data.X:
        ...
        counts+=1
#        if len(data.left_data)==counts:
        if(counts>303):
            print("오른쪽 데이터 이미지 생성 시작")
            imgtype='right'
            
            imgname="images_test/sub"+str(subject)+"_"+str(imgtype)+"_"+str(counts)+"_ch1.png"
            plot.specgram(pd.DataFrame(i).iloc[:,0], NFFT=64, Fs=250, noverlap=32)
            plot.axis('off')
    #        plot.show()
            plot.savefig(imgname, bbox_inches = 'tight', pad_inches = 0)
            imgname="images_test/sub"+str(subject)+"_"+imgtype+"_"+str(counts)+"_ch2.png"
            plot.specgram(pd.DataFrame(i).iloc[:,1], NFFT=64, Fs=250, noverlap=32)
            plot.axis('off')
    #        plot.show()
            plot.savefig(imgname, bbox_inches = 'tight', pad_inches = 0)
            imgname="images_test/sub"+str(subject)+"_"+imgtype+"_"+str(counts)+"_ch3.png"
